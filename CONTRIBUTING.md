# 🛠️ Contributing to Jirnyak/Bacevo
...
> Technology Foundation: `JavaScript / Canvas 2D / Math2D`
...
Bacevo Tensor Algebra & Isometric Collision Physics is engineered for maximum performance, deterministic state transitions, and zero computational slop. All contributions must respect existing subsystem boundaries and data flows:

```mermaid
graph TD
    A[World Coordinate Rigid Bodies] --> B[SAT Collision Solver]
    B -->|Minimal Translation Vector| C[Momentum Conservation Solver]
    C -->|Isometric 2:1 Projection| D[Canvas 2D Display]
```
...
### 1. 2:1 Isometric Ratio
* **Formal Requirement**: Screen-to-world transform must use exact 2:1 isometric matrix projections.
...
* **Formal Requirement**: Convex polygon collisions must compute minimal translation vectors.
* **Verification Protocol**: Automated unit test assertion + mathematical boundary check.
* **Failure Mode**: Immediate build rejection; PR cannot be approved without meeting this invariant.
...
* Primary Runtime: `JavaScript / Canvas 2D / Math2D`
...
```typescript
// ✅ CORRECT: 2:1 Isometric Projection Transformation
function worldToIso(x: number, y: number, z: number): [number, number] {
    const isoX = (x - y) * 0.866025; // cos(30 deg)
